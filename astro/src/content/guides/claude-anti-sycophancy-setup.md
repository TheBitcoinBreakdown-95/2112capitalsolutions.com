...
description: "Claude was built to agree with you. This setup configures it to push back, surface what could go wrong, and force structured disagreement before you commit to a plan."
pubDate: 2026-05-24
category: "AI"
level: "Intermediate"
draft: false
...
Those are not the same thing. AI assistants get rated on whether users approve of the response, not on whether the response is correct. So the model that survives is the one that agrees, encourages, and finds a way to deliver what you asked for. Even when what you asked for is wrong.

The result is an AI that will confirm your bad plan, miss the flaw in your argument, and call your half-baked idea worth pursuing. It will not lie to you exactly. It will just never say the thing that would have saved you three weeks of work.
...
This guide is a complete setup for fixing it. You'll configure Claude to push back by default, challenge assumptions before agreeing, and using a deliberate debate structure, surface the strongest case against your own ideas before you commit to them. Seven text files, written once, that change how every session starts.

If you've ever finished something with AI help and had someone immediately spot what was wrong with it... this is for you.

## In this guide

1. [What you're installing](#what-youre-installing) ... the seven files and what each one does
2. [Layer Zero: The context files](#layer-zero-the-context-files) ... CLAUDE.md, about-me.md, working-style.md
3. [The soul document](#the-soul-document) ... identity, not rules
4. [The output rules](#the-output-rules) ... default to skepticism
5. [The coaching layer](#the-coaching-layer) ... close the gaps at the start and end of tasks
6. [Using it: The debate personas](#using-it-the-debate-personas) ... structural perspective diversity
7. [Going deeper: Historical thinkers](#going-deeper-historical-thinkers) ... real reasoning traditions, not synthetic ones
8. [The one limit that doesn't go away](#the-one-limit-that-doesnt-go-away) ... model diversity for high-stakes work
9. [What this doesn't fix](#what-this-doesnt-fix) ... hallucination, capitulation, the underlying model
10. [Verifying it worked](#verifying-it-worked) ... the one test that actually answers the question

---

## What you're installing

Seven components, in the order they matter:
...
- **Debate personas** ... uses the pleasing instinct against itself by pointing it in two directions at once and reading the collision
...
The context files go in your project root. The personas go either in your project root (if you want them tailored to one project) or alongside the other rules in `~/.claude/rules/PERSONAS.md` (if you want them invocable everywhere). Either works. Global is the lower-friction default.
...
A practical shortcut: after your first substantive session on a new project, ask Claude to draft all three files from what it observed. It will capture your preferences from how you communicated during the session. Review and refine. Two minutes of work that saves you from writing them from scratch.
...
Create a `PERSONAS.md` file (either in your project root or in `~/.claude/rules/` for global use):
...
### Extending the base set

The six personas above cover most decisions. If you want richer coverage, add:

- **@UX-Agent** for error states, hidden complexity, anything user-facing
- **@Coherence** for terminology drift, internal consistency, cross-file ambiguity
- **@Scope-Guardian** for YAGNI defense and complexity creep
- **A domain-specific persona** (e.g., `@BTC-Agent` for anything touching Bitcoin protocol, custody, or settlement). The pattern is: name the perspective the base set doesn't already cover, declare its blind spot, write 3-5 questions it must always ask.

Add only what the base set doesn't catch. More personas isn't better. Orthogonal perspectives are.

---

## Going deeper: Historical thinkers
...
The rule: for high-stakes work like private data, payments, authentication, decisions that are hard to reverse, run the adversarial personas in a separate session with a different model. Claude implements. A fresh session reviews. The different model doesn't share the blind spots. That gap is the entire point.
...
Seven files. A context foundation, a baseline that fights the default agreeable behavior, and a persona system that turns it.
...
This setup compounds. Every session you run with it, you're directing a tool that's been configured once and works in your interest, not against it. The files are portable. They survive any Claude Code update. Point a new tool at the same folder and it picks them up.
